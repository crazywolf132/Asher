...
class CorpusUtilsTestCase(TestCase):
    ...
    def test_get_file_path(self):
        ...
        """
        path = self.corpus.get_file_path("samantha.corpus.english")
        self.assertIn(
            os.path.join("samantha", "corpus", "data", "english"),
            path
        )
    ...
    def test_load_corpus(self):
        corpus = self.corpus.load_corpus("samantha.corpus.english.greetings")

        self.assertEqual(len(corpus), 1)
        self.assertIn(["Hi", "Hello"], corpus[0])

    def test_load_corpus_general(self):
        corpus = self.corpus.load_corpus("samantha.corpus.english")
